...
import { Star, Coins, User, Sword, Trophy, Heart, Zap, Loader2, Edit, Settings, Briefcase, FileText, Eye, EyeOff, MessageSquare, BookOpen, ShieldCheck, Moon, UserCheck, LogOut, AlertCircle, Check, KeyRound } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { toggleStudentVisibility, updateStudentDetails, setMeditationStatus, forceStudentLogout, addShadowMark, removeShadowMark, resetStudentPassword } from '@/ai/flows/manage-student';
...
import { impersonateStudent } from '@/ai/flows/impersonate-student';
...
function ResetPasswordDialog({ student, teacherUid, onOpenChange }: { student: Student; teacherUid: string; onOpenChange: (open: boolean) => void }) {
    ...
    const handleReset = async () => {
        if (!newPassword || newPassword !== confirmPassword) {
            toast({ variant: 'destructive', title: 'Error', description: 'Passwords must match and not be empty.' });
            return;
        }
        setIsSaving(true);
        try {
            const result = await resetStudentPassword({ studentUid: student.uid, newPassword });
            if (result.success) {
                toast({ title: 'Password Reset', description: `Password for ${student.characterName} has been changed.` });
                onOpenChange(false);
            } else {
                throw new Error(result.error);
            }
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>Reset Password for {student.characterName}</DialogTitle>
                <DialogDescription>Enter a new password for this student. They will be able to log in with it immediately.</DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                    ...
                    <Input id="confirm-password" type="text" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
                </div>
            </div>
            <DialogFooter>
                <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
                <Button onClick={handleReset} disabled={isSaving || !newPassword || newPassword !== confirmPassword}>
...
export function StudentCard({ student, isSelected, onSelect, teacherUid, onSendMessage, hubs, chapters, companies, isOnline }: StudentCardProps) {
  ...
  const handleToggleVisibility = async (e: React.MouseEvent) => {
    ...
    const handleImpersonate = async () => {
        ...
        try {
            const result = await impersonateStudent(student.uid, auth.currentUser.uid);
    ...
    <>
      ...
      <Dialog open={isMeditationDialogOpen} onOpenChange={setIsMeditationDialogOpen}>
       ...
       <Dialog open={isPasswordDialogOpen} onOpenChange={setIsPasswordDialogOpen}>
          <ResetPasswordDialog student={student} teacherUid={teacherUid} onOpenChange={setIsPasswordDialogOpen} />
      </Dialog>
      <AlertDialog open={isImpersonateConfirmOpen} onOpenChange={setIsImpersonateConfirmOpen}>
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle>Impersonate {student.characterName}?</AlertDialogTitle>
                <AlertDialogDescription>
                    You will be temporarily logged in as this student with full access to their account. You will need to log back in as a teacher when you are finished.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={handleImpersonate}>
                    {isImpersonating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                    Yes, Impersonate
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <Card className={cn("shadow-lg rounded-xl flex flex-col overflow-hidden transition-all duration-300 relative", isSelected ? "ring-4 ring-primary scale-105" : "hover:scale-105", student.isHidden && "opacity-60 bg-gray-200")}>
           <div className="relative z-10 flex flex-col h-full bg-card/80 backdrop-blur-sm">
            ...
            <CardFooter className="p-2 bg-secondary/30 mt-auto grid grid-cols-3 gap-2">
                ...
                <Button className="w-full" variant="secondary" onClick={() => setIsImpersonateConfirmOpen(true)}>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                     ...
                     <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onSelect={() => setIsPasswordDialogOpen(true)}><KeyRound className="mr-2 h-4 w-4" /> Reset Password</DropdownMenuItem>
                        <DropdownMenuSeparator />
