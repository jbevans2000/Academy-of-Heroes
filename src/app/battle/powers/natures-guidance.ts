...
export async function useNaturesGuidance(input: UsePowerInput): Promise<{ success: boolean; message: string }> {
    const { teacherUid, studentUid, battleId, power } = input;

    try {
        // First, fetch the non-transactional data: the battle definition
        // We do this BEFORE the transaction to avoid reading inside it.
