...
export default function BattleSummariesPage() {
  ...
  const fetchSummaries = useCallback(async (user: User) => {
    ...
    try {
      ...
      const q = query(summariesRef, orderBy('startedAt', 'desc'));
  ...
  const handleRefresh = async () => {
    ...
    // The listener already keeps things up to date, but we can re-fetch for user assurance
    const summariesRef = collection(db, 'teachers', teacher.uid, 'savedBattles');
    const q = query(summariesRef, orderBy('startedAt', 'desc'));
    const querySnapshot = await getDocs(q);
    const summariesData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as SavedBattleSummary));
    setSummaries(summariesData);
