...
export function DashboardClient({ student, isTeacherPreview = false }: DashboardClientProps) {
  ...
  const magePowers = outOfCombatPowers['Mage'] || [];
  const healerPowers = outOfCombatPowers['Healer'] || [];
  const guardianPowers = outOfCombatPowers['Guardian'] || [];
    ...
    <TooltipProvider>
      ...
      <div className="p-4 md:p-6 lg:p-8">
        <div className="mx-auto max-w-4xl space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <Button size="lg" className="col-span-2 w-full py-8 text-lg justify-center bg-yellow-500 text-black hover:bg-yellow-600" disabled={isTrainingDone} onClick={handleDailyTrainingClick}>
                    <div className="flex items-center justify-center">
                        ...
                        <span>{isTrainingDone ? 'Training Complete for Today' : 'Daily Training'}</span>
                    </div>
                </Button>
                ...
                <Button size="lg" className="w-full py-8 text-lg" onClick={handleCheckCompany} disabled={isLoadingCompany}>
                     <div className="flex items-center justify-center">
          ...
          <div className="pt-6">
              {(student.class === 'Mage' || student.class === 'Healer' || student.class === 'Guardian') && !isTeacherPreview && (
                <div className="flex justify-center gap-4 mt-4">
                    {(student.class === 'Mage' ? magePowers : student.class === 'Healer' ? healerPowers : guardianPowers).map((power) => (
                        <TooltipProvider key={power.name}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="outline"
                                        size="icon"
                                        className="w-24 h-24 rounded-full border-4 bg-background/80 overflow-hidden disabled:opacity-50"
                                        onClick={() => handleOpenPowerDialog(power.name)}
                                        disabled={student.level < power.level}
                                    >
                                        <Image src={power.imageUrl} alt={power.name} width={100} height={100} className="rounded-full object-cover"/>
                                    </Button>
                                </TooltipTrigger>
                                <TooltipContent side="top" className="max-w-xs whitespace-normal">
                                    <p className="font-bold">{power.name} (Lvl {power.level})</p>
                                    <p>{power.description}</p>
                                    <p className="text-blue-500 font-semibold">Cost: {power.mpCost > 0 ? `${power.mpCost} MP` : 'Special'}</p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    ))}
