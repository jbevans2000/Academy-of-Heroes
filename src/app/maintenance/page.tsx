...
export default function MaintenancePage() {
    const [message, setMessage] = useState('The Academy of Heroes is under maintenance and will be back soon!');
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                ...
                console.error("Could not fetch maintenance message:", error);
            } finally {
                setIsLoading(false);
            }
        };
        ...
        >
             <div className="bg-black/70 p-8 rounded-lg shadow-2xl backdrop-blur-sm max-w-3xl space-y-4">
                {isLoading ? (
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold font-headline text-shadow-lg whitespace-pre-wrap">
                        {message}
                    </h1>
                )}
            </div>
